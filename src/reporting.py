...
def generate_reports(df_processed: pd.DataFrame, df_suggested_schedule: pd.DataFrame):
    ...
    plt.plot(df_combined_total_hours['Week'], df_combined_total_hours['Total_Hours_Threshold'],
             color='red', linestyle='--', label='Threshold') # Label will be duplicated for each type, but it's fine for clarity
    ...
    print(f"✅ Saved 'total_weekly_hours_comparison.png' to {LOCAL_REPORTS_DIR}") # And implied GCS_REPORTS_PATH logic
    ...
    plt.close() # Close plot to free memory
    print(f"✅ Saved 'nm_total_hours_comparison.png' to {LOCAL_REPORTS_DIR}") # And implied GCS_REPORTS_PATH logic
    ...
    if not df_moved_relays.empty:
        ...
        # For readability, group by original relay_id if multiple instances moved
        ...
        # Optionally, save this table to GCS as CSV too
        # save_to_gcs_gsv(moved_summary, f"{GCS_REPORTS_PATH}moved_relays_summary.csv") # Need a separate save_to_gcs_csv in utils
    ...
    if not df_pending_requests.empty and not df_moved_relays.empty:
        merged_moves_with_requests = df_moved_relays.merge(
            ...
            on=['Relay_ID', 'Store_ID'], # Merge on both Relay_ID and Store_ID for specific instances
            how='left'
        )
        # Check if the suggested move matches the pending request for that specific relay-store instance
        ...
        # Unhandled requests are those pending requests for instances that were NOT moved,
        # or were moved but not to the requested week.
        ...
        # Also include requests that were on a moved relay, but the suggested week didn't match
    ...
    # Identify new Relay_IDs in current year that were not in prev year
    # Need to consider unique Relay_IDs, not Relay_Store_Instances
